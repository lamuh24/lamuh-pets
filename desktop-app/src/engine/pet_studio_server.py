...
if getattr(sys, 'frozen', False):
    ROOT = Path(sys._MEIPASS)  # _internal/ where bundled assets live
    _appdata = os.environ.get('APPDATA', str(Path.home()))
    GENERATED_PETS_DIR = Path(_appdata) / 'LamuhPets' / 'generated_pets'
    _PET_EXE = Path(sys.executable).parent.parent / 'pet' / 'pet.exe'
else:
...
class PetStudioHandler(SimpleHTTPRequestHandler):
    ...
    def handle_spawn_pet(self):
        ...
        try:
            ...
            appdata_dir = str(Path(os.environ.get('APPDATA', str(Path.home()))) / 'LamuhPets')
            Path(appdata_dir).mkdir(parents=True, exist_ok=True)
            if _PET_EXE and _PET_EXE.exists():
                spawn_args = [str(_PET_EXE), str(sheet_path), "--name", display_name or pet_id]
            else:
